...
def parse_data(df, start, end, dim, hdim):
    tensor_dict = {}
...
def create_data(df, seq_len, dim, hdim):
    data_dict = {}
    labels_dict = {}
    count = 0
    for i in range(0, len(df.index) - seq_len + 1, seq_len):
        data_dict[count] = parse_data(df, i, i + seq_len, dim, hdim)
...
def get_data(path_to_csv, seq_len, dim, hdim, split, step):
    ...
    data, labels = create_data(df, seq_len, dim, hdim)
    return data, labels